...
$('body').on('click', function() {
	...
	if(menuOpen) {
		...
		menuItems.each(function(index) {
				setTimeout(function() {
					menuItems.eq(index).animated({'opacity': 0}, 300, function() {
	...
	$('nav > a:first-of-type').on('click', function(event) {
		...
		event.stopPropagation(); // Stop propagating the event to <span> and <nav>
		...
		// When menuOpen is false, try to show the entire menu.
			...
			menuItems.each(function(index){
				// Use setTimeout to delay an animation foe every single menuItem
				setTimeout(function() {
